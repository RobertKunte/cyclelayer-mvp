# CycleLayer V3.1b — Master Spec

**BraytonEngine + CycleLayerV3 für N-CMAPSS DS02**

*Single-Source-of-Truth für Claude-Code-Implementierung · Robert Kunte · Mai 2026 · Revision 3.1b*
...
V3.1b ist ein gezielter Patch der Phase C aus V3.1a. Die Phase C wurde umstrukturiert von "DS02 map tuning" zu einer methodisch sauberen dreistufigen Validierungs-Sequenz: Designpunkt-Anker-Konfiguration → External C-MAPSS Reference Check → DS02-Validierung. Hintergrund: Map-Tuning gegen die gleichen Sensoren, die später als Validierungsziel dienen, wäre methodisch problematisch (Selbst-Test) und im realen industriellen Anwendungsfall nicht reproduzierbar. V3.1b adressiert beides.

---

## Changelog V3.1a → V3.1b (Phase-C-Patch)

V3.1b ist ein chirurgischer Patch der Phase C. Architektur und alle anderen Phasen bleiben unverändert. Hintergrund der Änderung: Phase C in V3.1a war als "Map-Tuning auf N-CMAPSS DS02" formuliert, was zwei methodische Probleme barg — (a) Tuning gegen die gleichen Sensoren, die später als Validierungsziel dienen, ist ein Selbst-Test; (b) im realen industriellen Anwendungsfall existieren keine N-CMAPSS-äquivalenten Trainings-Sensoren, der Workflow wäre also nicht reproduzierbar.

| Nr. | Was geändert wurde | Wo |
|-----|--------------------|----|
| **B1** | Phase C umbenannt von "Map-Tuning" zu **"External C-MAPSS Reference Check + DS02-Validierung"**. Drei Sub-Phasen: C0 (Designpunkt-Anker), C1 (User-Guide-Vergleich), C2 (DS02-Stufe-3) | E.1, neuer Abschnitt C.3 |
| **B2** | **Ein** Flight Condition aus User Guide Tab. 1.4 (FC02) als Designpunkt-Anker reserviert; restliche 13 FCs sind echte Validierungspunkte | C.3a (neu) |
| **B3** | C-MAPSS-User-Guide-Validierung explizit nicht "external", sondern "external C-MAPSS reference" — gleiche Tool-Familie wie DS02, nur andere Daten-Schicht (Steady-State vs. Trajektorien) | C.3b (neu) |
| **B4** | Nf, Nc, Wf aus User Guide Tab. 1.4 sind **Inputs** in den Test-Aufruf, nicht Vergleichs-Outputs | C.3b |
| **B5** | Net Thrust **NICHT** als V3.1b-Validierungsziel — Düsen-Modellierung ist außerhalb V3.1b-Scope | C.3b |
| **B6** | EPR-Definition explizit als CMAPSS-Konvention (P50/P2) im Code-Kommentar fixiert | C.3b |
| **B7** | T48 als **Proxy** für T45 behandelt mit dokumentierter Toleranz, nicht als 1:1-Vergleich | C.3b |
| **B8** | Stufe 3a (DS02-Validierung) läuft **ohne weiteres Tuning** nach C0+C1; bei Verfehlung Stop, kein Tuning-Loop | C.3c |
| **B9** | DS02-Validierungs-Targets: T24, T30, T50, P30 (Total). Ps30 **nicht** als direktes Ziel (Total vs. Static unterschiedlich) | C.3c |
| **B10** | CLAUDE.md-Hard-Rule erweitert: keine versteckten Tuning-Loops, kein Re-Tuning gegen Test-Daten | E.2 |
...
## C.3 Stufe 3 — Reference-Engine Configuration und Validierungs-Sequenz

> **V3.1b:** Phase C wurde von "DS02 map tuning" zu einer dreistufigen Validierungs-Sequenz umstrukturiert. Reihenfolge: Designpunkt-Anker (C.3a) → External C-MAPSS Reference Check (C.3b) → DS02-Validierung (C.3c). Jede Sub-Phase hat ein klares Stop-Kriterium. **Keine versteckten Tuning-Loops, kein Re-Tuning gegen Test-Daten.**

Methodische Begründung: Es gibt drei klar getrennte Daten-Quellen mit unterschiedlichen Rollen. Diese Trennung ist Pitch-relevant — sie demonstriert, dass die BraytonEngine nicht gegen die Validierungsdaten optimiert wurde.

| Quelle | Rolle | Daten-Schnitt |
|--------|-------|---------------|
| Walsh & Fletcher / Kurzke / GasTurb generic | Konfigurations-Quelle (Map-Form, initial Designpunkte) | Literatur, generisch |
| C-MAPSS User Guide Tab. 1.4 | External C-MAPSS Reference (Designpunkt-Anker auf 1 FC, Validierung auf 13 FCs) | Steady-State Equilibria |
| N-CMAPSS DS02 | Test-Quelle | Flight-Trajektorien |

> **Wichtige Klarstellung zur Sprache:** Die User-Guide-Tab. 1.4 ist **nicht** eine vollständig unabhängige Quelle — sie stammt vom gleichen CMAPSS-Tool wie DS02, nur als andere Daten-Schicht (publizierte Steady-State-Equilibria statt simulierte Flight-Trajektorien). Der Begriff im Code und in Dokumentation ist daher konsistent **"external C-MAPSS reference check"**, nicht "independent validation". Echte unabhängige Validierung erfolgt erst in C.6 mit GasTurb.

### Stufe 3a — Designpunkt-Anker-Konfiguration (V3.1b Patch B2)

Eine einzige Flight Condition aus User Guide Tab. 1.4 wird als Designpunkt-Anker reserviert. Vorschlag: **FC02** (SLS, Mach 0.25, TRA 100%, Standard-Day, Sea Level).

Begründung der Wahl: FC02 liegt im "design point"-Bereich des CMAPSS 90K-Triebwerks (volle Drehzahl, Standard-Atmosphäre, niedrige Höhe). Map-Koeffizienten in generischen Maps sind typischerweise auf solche Designpunkte normiert, daher ist eine Anker-Skalierung an FC02 mathematisch und industriell üblich.

**Was passiert in 3a:**
- `Wc_fan_design`, `Nc_fan_design` und ggf. `c1`, `c2` der `inlet_flow`-Map werden so gewählt, dass für FC02 die User-Guide-Werte (Nf=2403 rpm, Nc=9084 rpm, Wf=7.085 pps) zu plausiblen `m_in` führen
- Diese Anpassung ist **einmalig**, dokumentiert mit Zahlenwerten in `configs/cyclelayer_v3.yaml` mit Kommentar "anchored to UserGuide FC02"
- Keine weitere Anpassung nach Abschluss von 3a

**Akzeptanz 3a:** für FC02 läuft BraytonEngine ohne NaN/Inf, alle Erhaltungssätze (Stufe 1) bestanden, T45 und EPR im plausiblen Bereich (innerhalb 25% von User-Guide-Werten an diesem Punkt).

### Stufe 3b — External C-MAPSS Reference Check (V3.1b Patch B3–B7)

> **V3.1b Patch B4:** Nf, Nc, Wf aus User Guide Tab. 1.4 sind **Inputs** in den Test-Aufruf, nicht Vergleichs-Outputs. Sie werden im Test-Code direkt aus der Tabelle gelesen und an `BraytonEngine.forward()` übergeben.

Die übrigen 13 FCs (alle außer FC02 — also FC01, FC03, FC04, ..., FC14) werden als **echte Validierungspunkte** verwendet. Jeder dieser Punkte ist eine unabhängige Beobachtung, an der die BraytonEngine bei θ=1.0 und mit der in 3a fixierten Konfiguration getestet wird.

**Test-Setup:**
```python
# tests/test_brayton_engine_userguide.py

# Aus C-MAPSS User Guide Tab. 1.4 (publiziert, NASA/TM-2007-215026)
USERGUIDE_FCS = {
    "FC01": {"alt_ft": 0,     "XM": 0.00, "Tsl_F": 59, "TRA_pct": 100,
             "Wf_pps": 6.835, "Nf_rpm": 2388, "Nc_rpm": 9051,
             "EPR_ref": 1.300, "T48_ref_R": 2072, "Fn_ref_lbf": 86336},
    # ... FC03 bis FC14 (FC02 ist Anker, nicht hier)
}

def test_userguide_reference_check():
    engine = BraytonEngine.from_yaml("configs/cyclelayer_v3.yaml")
    theta_healthy = torch.ones(1, 1, 5)   # all etas = 1.0

    errors = []
    for fc_name, fc in USERGUIDE_FCS.items():
        ops_imp = {...}        # construct from fc
        sens_imp = {"Nf_rpm": fc["Nf_rpm"],
                    "Nc_rpm": fc["Nc_rpm"],
                    "Wf_pps": fc["Wf_pps"]}
        ops_si  = to_si(ops_imp, sens_imp)
        sens_si = ops_si       # split as needed

        sensors_pred, diag = engine(ops_si, sens_si, theta_healthy)

        # T48 is HPT outlet (with cooling mix); we predict T45 (HPT outlet
        # before mix). T48 is treated as PROXY for T45 (V3.1b Patch B7),
        # not exact equality.
        T45_K = diag["T45"]
        T45_R = T45_K / RANK_TO_K
        rel_err_T45 = abs(T45_R - fc["T48_ref_R"]) / fc["T48_ref_R"]

        # EPR per CMAPSS convention: EPR = P50/P2 (V3.1b Patch B6).
        # User Guide Tab. 1.2 Index 3: "Engine pressure ratio (P50/P2)".
        EPR_pred = diag["P50"] / ops_si["P2_Pa"]
        rel_err_EPR = abs(EPR_pred - fc["EPR_ref"]) / fc["EPR_ref"]

        errors.append((fc_name, rel_err_T45, rel_err_EPR))

    # Acceptance criteria
    median_err_T45 = np.median([e[1] for e in errors])
    median_err_EPR = np.median([e[2] for e in errors])
    max_err_T45    = np.max([e[1] for e in errors])
    max_err_EPR    = np.max([e[2] for e in errors])

    assert median_err_T45 < 0.15, f"T45 median error {median_err_T45:.2%} > 15%"
    assert median_err_EPR < 0.15, f"EPR median error {median_err_EPR:.2%} > 15%"
    assert max_err_T45    < 0.25, f"T45 max error {max_err_T45:.2%} > 25%"
    assert max_err_EPR    < 0.25, f"EPR max error {max_err_EPR:.2%} > 25%"
```

**Akzeptanz 3b:**
- Median relativer Fehler an T45 (vs T48) und EPR < 15% über die 13 FCs
- Maximaler relativer Fehler < 25% (Outlier akzeptiert wenn dokumentiert: typisch hohe Höhe oder niedriges TRA)
- Kein systematischer Bias > 10% (z.B. immer T45 zu hoch) ohne Begründung
- **Net Thrust wird NICHT validiert** (V3.1b Patch B5) — Düsen-Modellierung außerhalb Scope

**Bei Verfehlung 3b:**
- Wenn alle 13 FCs verfehlen → Konfigurationsfehler in Phase 0 oder 3a, Robert involvieren
- Wenn 1–2 FCs verfehlen (typisch hohe Höhe oder niedriges TRA) → akzeptabel, dokumentieren
- Wenn systematischer Bias an einem Sensor → einmaliger Fix gegen FC02-Anker rechtfertigt, dokumentieren
- **NIEMALS:** mehrere Anpassungen gegen mehrere FCs, das wäre Tuning-Loop

### Stufe 3c — DS02-Validierung (V3.1b Patch B8–B9)

> **V3.1b Patch B8:** Diese Stufe läuft **OHNE weiteres Tuning** nach Abschluss von 3a und 3b. Wenn 3c verfehlt, ist das ein **Stop-Signal**, kein Tuning-Anlass.

Erst nachdem die Map-Konfiguration durch 3a fixiert und durch 3b extern validiert ist, wird gegen N-CMAPSS DS02 getestet. Die Sub-Stufen 3c-1, 3c-2, 3c-3 bleiben wie in V3.1a, aber mit präziseren Targets:

**3c-1 — Healthy-only:** θ=1.0 gegen DS02 RUL > 80. IoU > 0.7 auf den 4 Output-Sensoren.

**3c-2 — Degraded mit GT-Thetas:** θ_η_hpt und θ_η_lpt aus N-CMAPSS-GT (HPT_eff_mod, LPT_eff_mod als 1+mod). Andere θ=1.0. RUL < 30. IoU > 0.6.

**3c-3 — Held-out Units:** Map-Koeffizienten dürfen nicht auf Train-Units zurück angepasst werden. Held-out Test-Units müssen 3c-1/3c-2 ohne Re-Kalibrierung bestehen.

**Validierungs-Targets in 3c (V3.1b Patch B9):**
- T24, T30, T50 (Total Temperatures) — direkt vergleichbar
- P30 (Total Pressure) — direkt vergleichbar
- **Ps30 NICHT als direktes Vergleichsziel** — Total vs. Static unterscheiden sich um den dynamischen Druck. Falls Ps30 später benötigt, dann nur als Proxy.

**Bei Verfehlung 3c:**
- Stop, dokumentieren der Residuen pro Sensor und pro Unit
- Robert involvieren
- Kein Map-Tuning, kein "nur ein bisschen anpassen", kein hidden loop
...
## E.1 Phasenstruktur (V3.1b mit dreistufiger Phase C)

| Phase | Inhalt | Deliverable | Akzeptanz |
|-------|--------|-------------|-----------|
| **0 — Stunde 1** | **GasTurb-Konfiguration** | **CFM56-Klasse Standard-Turbofan, generische Maps + initial Designpunkte** | **Initial map_coeffs in YAML eingetragen** |
...
| **C0 — Tag 5 vormittags** | **Designpunkt-Anker an FC02 (User Guide Tab. 1.4)** | **Skalierungs-Faktoren in YAML, einmalig fixiert** | **FC02 läuft sauber, T45/EPR im 25%-Bereich** |
| **C1 — Tag 5 mittags** | **External C-MAPSS Reference Check auf 13 FCs** | **`tests/test_brayton_engine_userguide.py` grün** | **Median<15%, Max<25% für T45 und EPR** |
| **C2 — Tag 5 nachmittags** | **DS02-Validierung Stufe 3c-1/3c-2/3c-3 (KEIN Re-Tuning)** | **Stufe 3c grün auf train + held-out** | **IoU>0.7 healthy, IoU>0.6 degraded** |
| D — Tag 6 | Sensitivität + GasTurb-Cross-Check (echte unabhängige Quelle) | Stufe 4 + Stufe 6 grün | Vorzeichen korrekt, GasTurb-Diff <15% |
...
```
## Aktueller Fokus: CycleLayer V3.1b / BraytonEngine

Verbindliche Spec: docs/CycleLayer_V3.1b_Master_Spec.md (Mai 2026, Rev 3.1b).
...
5. Map-Koeffizienten und inlet_flow Parameter kommen aus GasTurb (Phase 0)
   und werden in C0 EINMALIG an FC02 angepasst. Danach FIXIERT.
...
17. Phase C0/C1/C2 sind STRIKT sequentiell:
    - C0 anchored Configuration ausschließlich an FC02 (nicht an mehreren FCs)
    - C1 Reference Check auf 13 FCs ohne Tuning-Loops
    - C2 DS02 Stufe 3c-1/3c-2/3c-3 ohne ANY Re-Tuning
    Bei Verfehlung in C1 oder C2: STOP, an Robert melden.
    Keine versteckten Tuning-Loops. Kein "nur ein bisschen anpassen". (B1, B8, B10)
18. Im User-Guide-Reference-Test: Nf, Nc, Wf sind INPUTS aus Tab. 1.4.
    Net Thrust ist NICHT validiert. EPR per CMAPSS-Konvention P50/P2.
    T48 ist PROXY für T45, nicht 1:1. (B4, B5, B6, B7)
...
- Düsen-Modellierung / Net Thrust Berechnung
...
- Map-Tuning gegen DS02 oder gegen mehrere User-Guide-FCs
...
### V4-Roadmap (NICHT in V3.1b implementieren)
...
- Düsen-Modul für Net Thrust Validierung
```
